...
** it does.
** <li><p>The third argument ("cnt") is the number of slots. Lookaside is
* disabled
...
** The first argument (the third parameter to sqlite3_db_config()) is an
* integer.
** The second argument is a pointer to an integer.  If the first argument is 1,
** then the option becomes enabled.  If the first integer argument is 0, then
* the
...
*/
SQLITE_API int sqlite3_set_errmsg(sqlite3* db, int errcode,
                                  const char* zErrMsg);
...
SQLITE_API int sqlite3_db_status(sqlite3*, int op, int* pCur, int* pHiwtr,
                                 int resetFlg);
SQLITE_API int sqlite3_db_status64(sqlite3*, int, sqlite3_int64*,
                                   sqlite3_int64*, int);
...
*/
SQLITE_API int sqlite3_snapshot_get(sqlite3* db, const char* zSchema,
...
** [SQLITE_ENABLE_SNAPSHOT] compile-time option is used.
*/
SQLITE_API int sqlite3_snapshot_open(sqlite3* db, const char* zSchema,
...
** returned.
*/
SQLITE_API int sqlite3changegroup_add_change(sqlite3_changegroup*,
...
struct Fts5ExtensionApi {
  ...
  int (*xTokenize_v2)(
      Fts5Context*, const char* pText, int nText, /* Text to tokenize */
...
struct fts5_api {
  ...
  /* Create a new tokenizer */
  int (*xCreateTokenizer_v2)(fts5_api* pApi, const char* zName, void* pUserData,
                             fts5_tokenizer_v2* pTokenizer,
                             void (*xDestroy)(void*));

  /* Find an existing tokenizer */
  int (*xFindTokenizer_v2)(fts5_api* pApi, const char* zName, void** ppUserData,
